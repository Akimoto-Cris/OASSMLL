...
if __name__ == '__main__':
    ...
    # Loss, Optimizer
    ###################################################################################
    class_loss_fn = nn.MultiLabelSoftMarginLoss(reduction='none').cuda()  # puzzleCAM
    ...
    for iteration in range(max_iteration):
        ...
        cams = make_cam(features)                # cams: [32, 20, 32, 32]
        mask = labels.unsqueeze(2).unsqueeze(3)  # mask: [32,20,1,1]
        cams_1 = (make_cam(features) * mask)     # cams: [32, 20, 32, 32]
        obj_cams = cams_1.max(dim=1)[0]
        ...
        for m in range(len(obj_cams)):

            cam_np = get_numpy_from_tensor(obj_cams[m])
            cam_np = cv2.resize(cam_np, (args.image_size, args.image_size), interpolation=cv2.INTER_LINEAR)
            max_index = np.unravel_index(np.argmax(cam_np), cam_np.shape)
            if max_index[0] == 0 or max_index[1] == 0:
                max_index = [256, 256]

            tiled_coordinates.append(max_index)
        ...
        for i in range(len(tiled_coordinates)):  # loop 32 times
            ...
            image_per_cut = []
            titled_index = tiled_coordinates[i]
            ...
            _, tiled_features = model(image_per_cut, with_cam=True)  # tiled_features: [4, 20, 16, 16]
            ...
            # Upper left: [1, 20, 3, 7]
            Upper_left = torch.nn.functional.interpolate(features_list[0], size=[merge_index[0], merge_index[1]],
                                                         scale_factor=None,
                                                         mode='nearest', align_corners=None)
            ...
            Bottom_right = torch.nn.functional.interpolate(features_list[3],
                                                           size=[32 - merge_index[0], 32 - merge_index[1]],
                                                           scale_factor=None,
                                                           mode='nearest', align_corners=None)
        ...
        p_class_loss = p_class_loss / len(tiled_coordinates)

        re_loss = re_loss / len(tiled_coordinates)
        ...
        def evaluate(loader):
            ...
            with torch.no_grad():
                ...
                for step, (images, labels, gt_masks) in enumerate(loader):
                    ...
                    scores, _ = model(images, with_cam=True)
